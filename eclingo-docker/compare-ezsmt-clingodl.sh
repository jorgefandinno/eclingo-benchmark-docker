...
# set variables
IMAGE_NAME=ezsmt-clingodl
SOLVER_1=clingo-dl
SOLVER_2=ezsmt-z3-idl
SOLVER_3=ezsmt-yices-idl
SOLVER_4=ezsmt-cvc5-idl

# build image
BUILD_COMMAND="docker build $BUILD_ARGS -t $IMAGE_NAME -f ezsmt-clingodl-comparison/Dockerfile ."
...
# run container
RUN_COMMAND="docker run --name $CONTAINER_NAME -e max_instances=3 -e benchmark=all $IMAGE_NAME"
printf "\nRunning: $RUN_COMMAND\n"
eval "$RUN_COMMAND"
...
mkdir -p results/$SOLVER_3
docker cp $CONTAINER_NAME:/root/eclingo-benchmark/running/benchmark-tool-$SOLVER_3/experiments/results/$SOLVER_3/$SOLVER_3.ods results/$SOLVER_3/
docker cp $CONTAINER_NAME:/root/eclingo-benchmark/running/benchmark-tool-$SOLVER_3/output/project/zuse/results/suite/ results/$SOLVER_3/

mkdir -p results/$SOLVER_4
docker cp $CONTAINER_NAME:/root/eclingo-benchmark/running/benchmark-tool-$SOLVER_4/experiments/results/$SOLVER_4/$SOLVER_4.ods results/$SOLVER_4/
docker cp $CONTAINER_NAME:/root/eclingo-benchmark/running/benchmark-tool-$SOLVER_4/output/project/zuse/results/suite/ results/$SOLVER_4/
